...
class TrajProcessor():
    ...
    def build_surf_compositions(self, build_csv=False,
        ...
        for i, index in enumerate(range(self.sim_indices[0], 1+self.sim_indices[1], 1)):
            if build_csv:
                ...
                if make_plots_bars or make_plots_lines:
                    self.plot_surf_comps(cus_df_bot=cus_df_bot, cus_df_top=cus_df_top, sim_index=index,
                                        make_plots_bars=make_plots_bars,
                                        make_plots_lines=make_plots_lines,
                                        path_save_plot=path_save_plot)
            else:
                cus_df_bot = pd.read_csv(filepath/f'cus_bot_{index}.csv')
                cus_df_top = pd.read_csv(filepath/f'cus_top_{index}.csv')
                if make_plots_bars or make_plots_lines:
                    self.plot_surf_comps(cus_df_bot=cus_df_bot, cus_df_top=cus_df_top, sim_index=index,
                                        make_plots_bars=make_plots_bars,
                                        make_plots_lines=make_plots_lines,
    ...
    def plot_surf_comps(self, cus_df_bot, cus_df_top, sim_index, make_plots_bars, make_plots_lines, path_save_plot):
...
def main():
    ...
    simulationGroup = TrajProcessor(sim_indices=sim_indices,
                                    ...
                                    skip_n_frames=200)
    # simulationGroup.build_surf_compositions(build_csv=True,
