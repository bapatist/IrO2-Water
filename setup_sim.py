...
def create_mix(water_box_files, adsorbates_file, slab_file):
    '''
...
'''
Task1:
    edit create mix such that it takes a new argument : coverage_type
    coverage_type can be random (the way it is right now) OR defined
    If defined:
        Bridge site:
            xO + yOH = 24
            ask for {'O': x, 'OH': y}
        Cus site:
            aOH2 + bOH + cO + dOO = 24
            ask for {'OH2': a, 'OH': b, 'O': c, 'OO': d}

    Task 1a: Finish simulator for random coverages only case
    Task 1b: Include the "defined starting coverage" case
'''
# Random only case:
sim_indexes         = int(input("Provide initial index: ")), int(input("Provide final index: "))
cwd                 = Path("/ptmp/nbapat/surf/coverage/rand_cov_iter3")
run_file            = "/ptmp/nbapat/surf/coverage/rand_cov_iter3/run.sh"
lmps_template_file  = "/ptmp/nbapat/surf/coverage/rand_cov_iter3/template.inp"

water_box_files     = glob.glob("/ptmp/nbapat/surf/coverage/rand_cov_iter3/W400_boxes/*xyz")
adsorbate_file      = "/ptmp/nbapat/surf/coverage/rand_cov_iter3/adsorbates.xyz"
slab_file           = "/ptmp/nbapat/surf/coverage/rand_cov_iter3/pristinex4.xyz"
...
for i in range(sim_indexes[0],sim_indexes[1],1):
    ...
    mix = create_mix(water_box_files=water_box_files,
                     adsorbates_file=adsorbate_file,
                     slab_file=slab_file)
