    ...
    def build_compositions_dfs(self, make_plots=True):
        for index in range(self.n_trajs):
            ...
            for atoms in self.trajs[index][::self.skip_n_frames]:
                ...
                append_this_bri = pd.DataFrame([{'Time': time, 'Energy':ener, 'OH2': len(bri_O_H2), 'OH': len(bri_O_H), 'O': len(bri_O_c), 'All': len(O_bri)}])
                ...
                append_this_cus = pd.DataFrame([{'Time': time, 'Energy':ener, 'OH2': len(cus_O_H2), 'OH': len(cus_O_H), 'O': len(cus_O_c), 'OO': len(cus_O_O), 'All': len(O_cus)}])
                cus_df.append(append_this_cus)
            bri_df = pd.concat(bri_df)
            cus_df = pd.concat(cus_df)
            filepath = Path.cwd()/'CSVs'  
            filepath.mkdir(parents=True, exist_ok=True)  
            bri_df.to_csv(filepath/'bri_df.csv')
            cus_df.to_csv(filepath/'cus_df.csv')
            if make_plots:
...
# %%
def main():
    objA = BinMe(sim_paths=[Path("sim_0/newmd.xyz")],
                init_struc_paths=[Path("sim_0/mix.xyz")],
                skip_n_frames=50)
    objA.build_compositions_dfs(make_plots=True)
