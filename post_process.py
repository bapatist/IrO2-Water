...
class TrajProcessor():
    ...
    def plot_density_profile(self):
        ...
        hist_O, bins_O = np.histogram(z_O, range=[0, zmax], bins=int(zmax/0.05))
        hist_H, bins_H = np.histogram(z_H, range=[0, zmax], bins=int(zmax/0.05))
...
#        rho_H = hist_H/len(traj_all)/Vbin/Norm_H2O #Output as g/ml
        ...
        plt.plot(bins_O, rho_O, color='k', label='O')
        plt.plot(bins_H, rho_H, color='gray', label='H')
        plt.xlabel('Z-distance in Ang.')
        plt.ylabel('Density of  in g/ml')
        plt.xlim(0,15)
        plt.legend()
...
def main():
    ...
    simulationGroup = TrajProcessor(sim_indices=sim_indices,
                                    ...
                                    skip_n_frames=10)
#    simulationGroup.build_surf_compositions(make_plots=True)
